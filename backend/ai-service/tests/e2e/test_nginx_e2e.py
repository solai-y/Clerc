...
def test_e2e_endpoint():
    ...
    url = "http://localhost/ai/e2e"
    expected_message = "AI Service is reachable"
    ...
    try:
        assert data.get("status") == expected_message
        print("[PASS] Message key matches expected value.")
    except AssertionError:
        print(f"[FAIL] Expected message '{expected_message}', got '{data.get('message')}'")
        raise
