...
def _normalize_sort(sort_by: str | None, sort_order: str | None) -> tuple[str, str]:
    # Map UI sort keys to child columns in raw_documents: name -> document_name, size -> file_size, date -> upload_date
...
def get_documents():
    ...
    try:
        ...
        sort_by = request.args.get('sort_by', type=str)        # "name" | "date" | "size"
        sort_order = request.args.get('sort_order', type=str)

        # Normalize sort parameters to supported keys
        ...
        # Log the intended parent-level order (implemented inside DatabaseService)
        if sort_by == "name":
            logger.info("Sorting parent by raw_documents.document_name %s with process_id tiebreaker", sort_order)
        elif sort_by == "size":
            logger.info("Sorting parent by raw_documents.file_size %s with process_id tiebreaker", sort_order)
        else:
            logger.info("Sorting parent by raw_documents.upload_date %s with process_id tiebreaker", sort_order)
        ...
        logger.info(
            "DB returned %s docs (error=%s) for sort_by=%s sort_order=%s",
            len(documents) if documents is not None else None, error, sort_by, sort_order
        )
